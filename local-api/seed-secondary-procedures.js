...
const SECONDARY_PROCEDURES = [
  {
    name: 'Alterar o solicitante para um novo e-mail',
    description:
      'Altera o proprietário de um cadastro de solicitante no sistema [INTERNAL-SYSTEM]. ' +
      'Documentações necessárias: RG (frente e verso), nome e ofício de alteração de usuário [INTERNAL-SYSTEM]. ' +
      'Procedimento realizado diretamente na interface web em internal-system.example.com/settings/applicants.',
    inputs: [],
    steps: [],
    order: 10,
  },
  ...
  },
  {
    ...
    description:
      ...
      'Validar previamente na tela de Histórico Completo e na aba de Reuniões do ciclo antes de executar. ' +
  ...
  {
    name: 'Alterar Porte do Projeto na Fechamento',
    description:
  ...
  },
  {
    ...
    description:
      'Parte 1: habilita o campo de alteração de segmento para o solicitante adicionando ' +
  ...
  },
  {
    ...
    description:
      'Habilita a aba de Complemento para o solicitante em indiretoEvaluations. ' +
  ...
  {
    name: 'Data Limite do Envio do Complemento da Fechamento (Fechamento Antigo — Análise Exauriente)',
    description:
      'Ajusta a data limite para envio do complemento em secondaryAccountabilityThoroughVersion ' +
      '(campo complementLimitDate). Para Análise Sumária da Fechamento Antigo usar secondaryAccountabilityVersion. ' +
    ...
    steps: [
      {
        collection: 'secondaryAccountabilityThoroughVersion',
        operation: 'backup',
  ...
  {
    name: 'Exclusão do novo fechamento',
    description:
      'Exclui uma Fechamento criada indevidamente. ' +
  ...
  {
    name: 'Expandir data limite do complemento de informações em novo fechamento',
    description:
      'Expande a data limite para complemento no novo fechamento Secundário: atualiza status para PROPONENT_PENDING_COMPLEMENT ' +
  ...
  },
  {
    ...
    description:
      'Exclui documentos enviados equivocadamente em submissionDocuments e reabilita o envio pelo solicitante. ' +
  ...
  {
    name: 'Queries - Novo fechamento',
    description:
      'Consultas de aggregation MongoDB para extração de informações do novo fechamento Secundário na collection accountabilityIndireto. ' +
      '(1) Fechamentos Iniciadas: match status PROPONENT_DRAFT + lookup submissions + filtro section Secundário não deprecated. ' +
      '(2) Fechamentos Enviadas: match sentDate != null. ' +
      '(3) Fechamentos Iniciadas e Enviadas combinadas. ' +
  ...
  },
  {
    ...
    description:
      'Retorna a avaliação ao revisor para correção do parecer: altera status em indiretoEvaluations ' +
  ...
  {
    name: 'Retornar Avaliação para o Revisor',
    description:
      'Retorna projeto cancelado automaticamente ao revisor para inclusão do parecer: altera status em ' +
  ...
  },
  {
    ...
    description:
      ...
      'Validar no front-end se a alteração é para documentos do projeto ou do solicitante.',
    inputs: [
      { key: 'submissionId', label: '_id da submission (ObjectId)', type: 'objectId', required: true },
    ],
    steps: [
  ...
  {
    name: 'Retornar para o avaliador - Fechamento (antiga)',
    description:
      'Reabre o relatório do fechamento antigo para a equipe de Fechamentos do Secundário iniciar a análise exauriente: ' +
      'altera proponentStatus para COMPLEMENTSENT e status para DRAFT em secondaryAccountabilityThoroughVersion, ' +
      'fazendo o sistema interpretar que o complemento solicitado foi enviado pelo solicitante.',
    inputs: [
      { key: 'submissionId', label: 'submissionId do documento (ObjectId)', type: 'objectId', required: true },
    ],
    steps: [
  ...
  {
    name: 'Retornar para o Solicitante Enviar o Complemento do fechamento (Novo Fechamento)',
    description:
      'Retorna o projeto para o solicitante enviar complementos no novo fechamento: atualiza status para ' +
  ...
  {
    name: 'Retornar fechamento criada erroneamente',
    description:
      ...
      'remover o campo secondaryPostApproval.isCompleted e definir secondaryPostApproval.isActive como true ' +
    ...
    steps: [
      ...
      },
      {
        ...
        filter: { submissionNumber: '{{submissionNumber}}' },
        update: {
          $unset: { 'secondaryPostApproval.isCompleted': '' },
          $set: { 'secondaryPostApproval.isActive': true },
        },
      },
    ],
    order: 210,
...
async function main() {
  ...
  const existing = await db.collection('pipeon_procedures')
    .find({ projectId, nucleo: 'Secundário' })
    .toArray();
  const existingNames = new Set(existing.map((p) => p.name));

  const toInsert = SECONDARY_PROCEDURES.filter((p) => !existingNames.has(p.name)).map((p) => ({
    ...p,
    nucleo: 'Secundário',
    projectId,
    isActive: true,
  ...
  if (toInsert.length === 0) {
    console.log('Todos os procedimentos Secundário já estão cadastrados. Nenhuma inserção necessária.');
    await client.close();
    return;
  }

  const result = await db.collection('pipeon_procedures').insertMany(toInsert);
  console.log('\n' + result.insertedCount + ' procedimento(s) Secundário inserido(s):');
